...
class GS_Linear:
    """
        Class representing analytic solutions to the Grad-Shafranov equation
        with the Freidberg-Cerfon-Solovev profiles.
    """
    ...
    def psi_func(self, x, y):
        ...
        if not self.beta_limit:
            cj_UH = UH * self.cj[0]
            UP = self.A * APart+nonAPart

        elif self.beta_limit:
    ...
    def get_point_slope_dx(self, x, y):
        ...
        # If calculating single null, add 5 polynomials for up-down asymmetry
        if self.divertor == 1:
